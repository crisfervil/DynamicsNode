...
import xmlEncode = require('xml-escape');

export class SoapXmlHelper
{

  private static buildExecuteRequest(soapRequestBodyXml) {
  ...
  static buildFetchAttributeXml(columns) {
      var attributes = [];
  ...
  static buildGetByIdFetchXml(id, entityname, columns) {
  ...
  static getRetrieveRequest(id:string, entityname:string, columns?:string[]) {

    var requestColumns:string;
    if(columns){
       requestColumns = columns.map(
  ...
  static getAssignXml(id, entityname, assigneeId, assigneeEntityName) {
